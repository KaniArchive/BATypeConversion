...
public class ExportTypeConversionGlobal extends GhidraScript {
    ...
    protected void run() throws Exception {
        ...
        try (FileWriter writer = new FileWriter(f)) {
            ...
            java.util.HashSet<String> declaredUnks = new java.util.HashSet<String>();
			StringBuilder unkDefinitions = new StringBuilder();
			java.util.regex.Pattern pattern = java.util.regex.Pattern.compile("UNK_([0-9a-fA-F]+)");

			for (DecompiledFunction func : results) {
				String cCode = func.getC();
				java.util.regex.Matcher m = pattern.matcher(cCode);
				while (m.find()) {
					String unkName = m.group(0);
					if (!declaredUnks.contains(unkName)) {
						declaredUnks.add(unkName);
						long addr = Long.parseLong(m.group(1), 16);
						ghidra.program.model.address.Address address = currentProgram.getAddressFactory().getDefaultAddressSpace().getAddress(addr);
						
						byte[] bytes = new byte[8192]; 
						try {
							currentProgram.getMemory().getBytes(address, bytes);
							unkDefinitions.append("unsigned char ").append(unkName).append("[] = {");
							for (int j = 0; j < bytes.length; j++) {
								unkDefinitions.append(String.format("0x%02x,", bytes[j]));
							}
							unkDefinitions.append("0};\n");
						} catch (ghidra.program.model.mem.MemoryAccessException e) {
                            unkDefinitions.append("unsigned char ").append(unkName).append("[8192]; // Failed memory read\n");
                        }
					}
				}
            }

            writer.write(unkDefinitions.toString());
            writer.write("\n");
